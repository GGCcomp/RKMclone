...
const OurVisionSection = () => {
  ...
  const navigateToVisionPage = () => {
   ...
   const getActiveTabData = () => {
    switch (activeTab) {
      case 'media':
        return mediaItems.length > 0 ? mediaItems.slice(0, 3) : []; // Show only the first 3 items
      case 'programs':
        return programs.length > 0 ? programs.slice(0, 3) : [];
      case 'projects':
        return projects.length > 0 ? projects.slice(0, 3) : [];
      default:
        return [];
    }
  };
    ...
    >
      ...
      <div className="space-y-3 p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-text-secondary">
            {formatDate(program.startDate || program.createdAt)}
          </span>
        </div>
        <h3 className="text-lg font-heading font-semibold text-text-primary">
    ...
    >
      ...
      <div className="space-y-3 p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-text-secondary">
            {formatDate(project.expectedStartDate) || formatDate(project.createdAt)}
          </span>
        </div>
        <h3 className="text-lg font-heading font-semibold text-text-primary">
        ...
        <div className="relative">
          ...
          {tabData.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {tabData.slice(0, 3).map(item => renderItem(item))}
            </div>
          ) : (
            renderEmptyState()
          )}
